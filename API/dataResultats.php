...
class DataResultats {
    ...
    public function createPartie($date, $tempsReactionJ1,$identifiant,$plotsTotaux,$plotsAppuyes) {
        // Initialisation de la réponse
        $reponse = array('statut' => 1, 'message' => 'ok'); // 1 pour validée
        ...
        // Création de la requête
        $sql = 'INSERT INTO partie (date, tempsReactionJ1, identifiant,plotsTotaux,plotsAppuyes) VALUES ("'.$date.'", "'.$tempsReactionJ1.'", "'.$identifiant.'", "'.$plotsTotaux.'", "'.$plotsAppuyes.'")';
        $s = $this->bdd->prepare($sql);
        
        // Exécution de la requête
        $res = $s->execute([$date, $tempsReactionJ1, $identifiant,$plotsAppuyes,$plotsTotaux]);
